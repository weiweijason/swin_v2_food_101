        ...
        for index, (depth, number_of_head) in enumerate(zip(depths, number_of_heads)):
            self.stages.append(
                SwinTransformerStage(
                    dim=embedding_channels * (2 ** max(index - 1, 0)),
                    depth=depth,
                    ...
                    mlp_ratio=ff_feature_ratio,
                    drop=dropout,
                    attn_drop=dropout_attention,
                    dropout_path=dropout_path[sum(depths[:index]):sum(depths[:index + 1])],  # 修改參數名稱：drop_path -> dropout_path
